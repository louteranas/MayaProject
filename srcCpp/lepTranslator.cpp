...
public:
	...
	void recursiveJoint(Joint* currentJoint, std::array<double, 3> offset);
	//Set the keyframes of the skeleton
	void setAllKeyframes(std::vector<Joint*> lJoints);
	void setKeyframes(Joint* joint);
	...
	MPxFileTranslator::FileAccessMode mode)
{
	...
	setAllKeyframes(lJoints);
	return rval;
}

MStatus BvhTranslator::displayJoints(std::vector<Joint*> lJoints) {
	MStatus rval(MS::kSuccess);
	std::vector<AnimCurve> rootDofs = lJoints[0]->_dofs;
	std::array<double, 3> offset = { rootDofs[3]._values[0], rootDofs[4]._values[0], rootDofs[3]._values[5] };
	int taille = lJoints[0]->_children.size();
	MGlobal::displayInfo(MString("YO : ") + taille);
	recursiveJoint(lJoints[0], offset);
	return rval;
}

void BvhTranslator::recursiveJoint(Joint* currentJoint, std::array<double, 3> offset) {
	std::array<double, 3> newOffset = { offset[0] + currentJoint->_offX, offset[1] + currentJoint->_offY, offset[2] + currentJoint->_offZ };
	MString cmdString = MString("joint -p ");
	cmdString += newOffset[0];
	cmdString += " ";
	cmdString += newOffset[1];
	cmdString += " ";
	cmdString += newOffset[2];
	cmdString += ";";
	MGlobal::executeCommand(cmdString);
	int taille = currentJoint->_children.size();
	for (std::vector<Joint*>::iterator it = currentJoint->_children.begin(); it != currentJoint->_children.end(); ++it) {
		MString selectString = MString("select -r joint");
		selectString += currentJoint->id_liste;
		MGlobal::executeCommand(selectString);
		recursiveJoint((*it), newOffset);
	}
	return;
}

void BvhTranslator::setAllKeyframes(std::vector<Joint*> lJoints) {
	for (std::vector<Joint*>::iterator it = lJoints.begin(); it != lJoints.end(); ++it) {
		setKeyframes(*it);
	}
}

void BvhTranslator::setKeyframes(Joint* joint) {
	MString message("SALUT : ");
	message += joint->id_liste;
	message += " ";
	message += joint->_name.c_str();
	MGlobal::displayInfo(message);
	for (unsigned int idof = 0, nbDofs = joint->_dofs.size(); idof < nbDofs; idof++) {
		for (unsigned int iframe = 0, nbFrames = joint->_dofs[idof]._values.size(); iframe < nbFrames; iframe++) {
			int time = (iframe+1) * 10;
			MString cmdString("setKeyframe -t ");
			cmdString += time;
			cmdString += " -v ";
			cmdString += joint->_dofs[idof]._values[iframe];
			cmdString += " -at \"";
			if (idof==3) cmdString += "translateX\" ";
			if (idof==4) cmdString += "translateY\" ";
			if (idof==5) cmdString += "translateZ\" ";
			if (idof==0) cmdString += "rotateX\" "; 
			if (idof==1) cmdString += "rotateY\" ";
			if (idof==2) cmdString += "rotateZ\" ";
			cmdString += "joint"; cmdString += joint->id_liste;
			cmdString += ";";
			MGlobal::executeCommand(cmdString);
			if (joint->id_liste == 19 && (idof == 0 || idof == 1 || idof == 2)) {
				MGlobal::displayInfo(cmdString);
			}
		}
	}
}
